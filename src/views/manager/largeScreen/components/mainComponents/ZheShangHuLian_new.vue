      ...
      <el-table class="zshl_table_wrap" :data="tableData" :span-method="splitCellMethod" align="left">
        ...
        <el-table-column label="项目进度">
          <template slot-scope="scope">
            <el-tooltip effect="dark" :content="scope.row.project_progression.substr(2, 8)" placement="right">
              <div class="hltb_item_wrap">
                <!-- <img src="../mapComponents/mapImg/zshl/zshltbicopng.png" alt="" /> -->
                <zhe-shang-hu-lian-chart ref="chart2Ref" :chart-data="zheShangHuLianData" :height="'215px'" />
                <span>{{ scope.row.project_progression.substr(0, 1) }}</span>
              </div>
              <!-- <el-button>{{ scope.row.project_progression.substr(0, 1) }}</el-button> -->
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
import ZheShangHuLianChart from '../../charts/ZheShangHuLianChart'

export default {
  components: {
    BrainTable, ZheShangHuLianChart
  },
    ...
    handleBrainTbChange() {
      this.$el.addEventListener('click', e=> {
        ...
        if(target.nodeName === 'SPAN') {
          ...
          pointDom = target.querySelector('#proPer')
        }

        console.log(pointDom)

        // console.log(trNode.querySelector('#proPer'))
      })
    },
