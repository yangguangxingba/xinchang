<template>
  <section class="large_screen_wrap">
    ...
    <div class="lagscn_top">
      ...
      <!-- 返回首页按钮 -->
      <button class="to_index" @click="toLast(0)"></button>

      <!-- 返回二级按钮 -->
      <button class="to_last" @click="toLast(1)"></button>

      <!-- 全屏 -->
      <button class="full_screen_wrap" @click="fullScreen"></button>

      <!-- 用户信息 -->
      <el-dropdown trigger="click" @command='setDialogInfo' class="user_wrap">
      ...
      <div class="lagscncot_left">
        ...
        <div class="lgcp_wrap revenue_profit">
          ...
          <div class="lgcp_cont">
            <div class="lgcpcot_wrap justify_cot">
              <div class="lgcpcot_l">
                <revenue-profit-chart-1 ref="chart3Ref" :chart-data="revenueProfitData1" 
                                                        :height="'135px'" :ysSubTitle="ysSubTitle" 
                                                        @setRevenueProfitChart2Data="setRevenueProfitChart2Data" />
              </div>
              <div class="lgcpcot_r">
                <revenue-profit-chart-2 ref="chart4Ref" :chart-data="revenueProfitData2" 
                                                        :height="'135px'" />
              </div>
            </div>
          </div>
          ...
          <div class="lgcp_cont">
            <div class="lgcpcot_wrap">
              <div class="industrial_table justify_cot">
                ...
                <div class="lgcpcot_r">
                  <brain-table class="industrial_table_wrap" :tableData="industrialTableData2" 
                                                             :columns="industrialTableColumns" 
                                                             :height="'162px'" :isClick="true" 
                                                             :listLoading="industrialListLoading" 
                                                             @getRowData="getCompanyData" />
                </div>
              </div>
                    ...
                    <dl>
                      <dt>
                        <ul class="justify_cot num_txt_wrap">
                          ...
                          <li>活跃会员<span>{{ businessLeftData.active_cnt }}</span>万人</li>
                        </ul>
                      </dt>
                      <dd>
      ...
      <div class="lagscncot_center">
        <div class="lgstcnt_wrap">
          ...
          <div class="lgstcnt_bot_wrap">
            <!-- 主体中间内容 -->
            <large-screen-main ref="lgcnMainRef" @setZswhData="setZswhData" 
                                                 @getMarkerData="setRightPageData" 
                                                 @getMapAreaData="setRightPageData" 
                                                 @getAll="getAll" />
          </div>
        </div>
      </div>
                    ...
                    <dl class="f-cb">
                      ...
                      <dd class="f-fl">
                        <ul class="compare num_txt_wrap">
                          ...
                          <li><span>{{ businessRightData.totle_rqcl }}</span>辆</li>
                        </ul>
                      </dd>
                    </dl>
                  </div>
          ...
          <div class="lgcp_cont">
            <div class="lgcpcot_wrap">
              ...
              <div class="lgcprit_item justify_cot lgcprit_res">
                <div class="lgcpcot_l">
                  <service-area-chart ref="chart11Ref" :chart-data="serviceAreaData" :height="'215px'" />
                </div>
                <div class="lgcpcot_r">
                  <brain-table class="service_area_table_wrap" :height="'215px'" 
                                                               :tableData="serviceAreaTableData" 
                                                               :columns="serviceAreaTableColumns" 
                                                               :listLoading="serviceAreaListLoading" 
                                                               :isClick="true" 
                                                               @getRowData="setRightPageData" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="lgcpcot_wrap">
              ...
              <div class="lgcprit_item justify_cot lgcprit_res">
                <div class="lgcpcot_l">
                  ...
                  <div class="ssse">
                    <brain-table ref="publicServiceTableRef" class="public_service_table_wrap" :height="'215px'" 
                                                                                               :isFixed="true" 
                                                                                               :tableData="publicServiceTableData" 
                                                                                               :columns="publicServiceTableColumns" 
                                                                                               :listLoading="publicServiceListLoading" />
                  </div>
                </div>
                <div class="lgcpcot_r">
                  <div class="tip_title">
...
export default {
  ...
  data() {
    return {
      ...
      // 服务区经营数据
      serviceAreaTableColumns: [{ prop: 'area_anme', label: '区域', align: 'center' }, { prop: 'newRevenueAmount', label: '累计对客营收(万元)', align: 'center' }, { prop: 'YearOnYear', label: '同比', align: 'center' }],
      // 公共服务数据
      publicServiceTableColumns: [{ prop: 'servicepart_name', label: '服务区', align: 'center' }, { prop: 'create_time', label: '时间', align: 'center' }, { prop: 'complaint_content', label: '投诉内容', isShowTool: true }],
  ...
  methods: {
    ...
    // 获取浏览器参数 - token,suffixPhone,userName
    getUrlQuery() {
      let query = location.search.substr(1)
      // let query = 'http://commerce.decision.cncico.com/publish/veiwpage?token=Bearer%20eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..dKstNDVwAEnxCAiIv2Eu18QYHF_byFS7TsxD9Sc5YdQ5RmHBeQg12EodX8C2Nj6l4TVZgZCcvTD9gTq4SvzDTAQnO8OfEwbKXL0W3qxQJHFp8z8Anpm7GZFZkfugcbAqknV86Z3J9I7VQkqTi8qeHNFfW_OYU4eP7lB9iihUSTk&suffixPhone=0582&userName=%25E4%25BD%2595%25E8%2589%25AE%25E6%2598%25A5'
      this.urlObj = this.globalUtils.urlToObj(query)
      if(this.urlObj.userName) {
        this.urlObj.userName = decodeURI(this.urlObj.userName)
      } else {
        this.urlObj.userName = ''
      }
    },
    // 返回二级看板
    toLast(val) {
      ...
      if(val === 0) {
        ...
        path = `http://decision.cncico.com/index?token=${this.urlObj.token}`
        window.location.href = path
      } else {
        /* 二级首页地址：http://commerce.decision.cncico.com/publish/veiwpage
        参数：token,suffixPhone,userName */
        // console.log("返回二级")

        path = `http://commerce.decision.cncico.com/publish/veiwpage?token=${this.urlObj.token}`
        window.location.href = path
      }
    },
    // 全屏
    ...
    setDialogInfo(cmditem) {
      ...
      switch (cmditem) {
        case "logout":
          ...
          // this.$router.push('*')
          // this.$router.push('*')
          let path = `http://decision.cncico.com/user/login`
          window.location.href = path
          break;
      }
    },
    ...
    async renderLeft() {
      ...
      // console.log(this.riskDataData)
      /* console.log("x轴", this.riskDataData.fieldData)
      console.log("资产负债", this.riskDataData.yData1)
      console.log("存货增长率", this.riskDataData.yData2)
      console.log("应收账款率", this.riskDataData.yData3) */
      // 获取风险数据
    ...
    async getRevenueProfitList() {
      ...
      if (this.revenueProfit === 1) {
        ...
        liRunChart1.forEach(item => {
          this.revenueProfitData1.fieldData.push({ 
            value: Math.abs(item.month_profitsYear.toFixed(2)),
            name: item.business_type, 
            percentage: item.Result.toFixed(2), 
    ...
    async getRevenueLineData(business_id) {
      ...
      // console.log('营收折线处理后数据', yingShouChart2After, business_id)
      // 营收与利润结构 - 右
      ...
      // console.log(this.revenueProfitData2.yData1)
      ...
      // console.log(this.revenueProfitData2.yData2)
      ...
      // console.log(this.revenueProfitData2.yData3)
    },
    // 利润折线
    async getProfitLineData(business_id) {
      ...
      // console.log('利润折线处理后数据', liRunChart22After, business_id)
    ...
    async renderCenter() {
      ...
      // 本年累计对客营收
      let bnljdkysData = await lgTipApi.getBnljdkysData()
      // console.log("本年累计对客营收", bnljdkysData)
      this.businessCenterData = Object.assign(this.businessCenterData, indexMapTopData[0], bnljdkysData[0])
      // console.log(this.businessCenterData)
    ...
    getAll() {
      ...
      this.renderRightServerData()  // 渲染服务区(总数据)
    ...
    renderFlowData2(flowData) {
      ...
      this.businessRightData = Object.assign(this.businessRightData, flowData[0])
      if(this.businessRightData.car_numday && this.businessRightData.car_numYear) {
        this.businessRightData.totle_rqcl = this.businessRightData.car_numday + this.businessRightData.car_numYear
      }
    },
    // 今日入区率
...
.large_screen_wrap {
  ...
  .lagscn_top {
    ...
    .full_screen_wrap {
      ...
      width: 21px;
      height: 21px;
      background: url('../../../assets/images/largescreen/qp.png') top no-repeat;
      background-size: 100%;
      border: none;
    }
